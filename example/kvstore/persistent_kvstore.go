...
func NewPersistentStorageApplication(dbDir string) *PersistentStorageApplication {
	db := dbm.NewDB("kvstore", "leveldb", dbDir)
	...
	return &PersistentStorageApplication{
		app:    &StorageApplication{state: stateTree},
		db:     db,
...
func (app *PersistentStorageApplication) execValidatorTx(tx []byte) types.Result {
...
func (app *PersistentStorageApplication) updateValidator(v *types.Validator) types.Result {
